...
$con=mysqli_connect("localhost","root","","hmsproject");
  ...
  if (isset($_POST['app-submit'])) {
    ...
    // 🔴 NEW: Night shift restriction check
    $prevDate = date('Y-m-d', strtotime($appdate . ' -1 day'));
    $shiftCheck = mysqli_prepare($con, "SELECT * FROM doctor_roster 
                                        WHERE doctor_username = ? 
                                        AND shift = 'Night' 
                                        AND (date = ? OR date = ?)");
    mysqli_stmt_bind_param($shiftCheck, "sss", $doctor, $appdate, $prevDate);
    mysqli_stmt_execute($shiftCheck);
    $result = mysqli_stmt_get_result($shiftCheck);
    if (mysqli_num_rows($result) > 0) {
        echo "<script>alert('Doctor is not available on this date due to a prior night shift. Please choose a different date or doctor.');</script>";
        return;
    }
    ...
    if (date("Y-m-d", $appdate1) >= $cur_date) {
      if ((date("Y-m-d", $appdate1) == $cur_date and date("H:i:s", $apptime1) > $cur_time) or date("Y-m-d", $appdate1) > $cur_date) {
        ...
        if (mysqli_num_rows($check_query) == 0) {
          ...
          echo "<script>alert('Doctor is already booked for this time. Please choose another slot.');</script>";
        }
      } else {
        echo "<script>alert('Select a time in the future.');</script>";
      }
    } else {
